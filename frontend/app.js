...
const LABEL_DEFINITIONS=[...SEATS.map((s,i)=>({key:`seat-${i}`,name:s.name,group:'좌석 이름',hint:`좌석 ${i+1} · ${s.name}`})),...ASSETS.map(a=>({...a,group:a.key.startsWith('room-')?'공간 이름':'고정 자산 이름',hint:a.name})),...CABINETS.map(c=>({key:`cabinet-${c.id}`,name:`캐비닛 ${String(c.id).padStart(2,'0')}`,group:'캐비닛 이름',hint:`도면 번호 ${c.id}`}))];
...
function validateAssets(value){
 ...
 if(!a||typeof a.key!=='string'||!/^custom-[a-z0-9-]{1,80}$/.test(a.key)||ids.has(a.key)||typeof a.name!=='string'||!a.name.trim()||a.name.length>60||!['storage','table','printer','purifier'].includes(a.type)||!Number.isFinite(a.x)||!Number.isFinite(a.y)||a.x<14||a.x>277||a.y<18||a.y>731)throw Error('추가한 자산의 이름·종류·위치가 올바르지 않습니다.');
 ids.add(a.key);return {key:a.key,name:a.name.trim(),type:a.type,x:a.x,y:a.y,w:20,d:24};});
...
function selectCabinet(id,focusMap=false){selected=id;renderList();renderDetail();updateHighlights();if(focusMap)focusPoint(CABINETS.find(c=>c.id===id));}
...
wall(11,start,3,513-start,9,'outer-wall');
 ...
 for(const a of assetState.custom){const html=a.type==='table'?table(a.x,a.y,a.w,a.d):a.type==='printer'?printer(a.x,a.y):a.type==='purifier'?purifier(a.x+10,a.y+12):box(a.x,a.y,a.w,a.d,30,'#c6d7c3','#ebeddf','#b9cbb0','asset-top');asset(a.key,a.x,a.y,a.w,a.d,html,a.x+10,a.y+12,44);}
...
$('assetAddForm').onsubmit=e=>{e.preventDefault();const name=$('assetName').value.trim();if(!name){$('assetName').setCustomValidity('자산 이름을 입력해 주세요.');$('assetName').reportValidity();return;}if(assetState.custom.length>=200){showToast('추가 자산은 최대 200개까지 등록할 수 있습니다.');return;}pendingAsset={name,type:$('assetType').value};$('assetsDialog').close();$('placementBar').hidden=false;$('map').classList.add('placing');$('tooltip').hidden=true;};
...
function placeAsset(e){const q=svgPoint(e);let x=q.x,y=q.y;if(mode==='iso'){const u=(q.x/.8660254+2*q.y)/2,v=(2*q.y-q.x/.8660254)/2;[x,y]=[[u,v],[v,-u],[-u,-v],[-v,u]][rotation];x+=155;y+=520;}if(x<14||x>297||y<18||y>755){showToast('지도 안의 위치를 선택해 주세요.');return;}const a={...pendingAsset,key:`custom-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,x:Math.min(277,Math.max(14,Math.round(x-10))),y:Math.min(731,Math.max(18,Math.round(y-12))),w:20,d:24};assetState.custom.push(a);labels[a.key]=a.name;cancelPlacement();persist();renderMap();refresh();$('assetAddForm').reset();showToast(storageProblem||'고정 자산을 추가했습니다.');}
