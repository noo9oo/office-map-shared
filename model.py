...
def validate(payload):
    ...
    custom_ids = set()
    for a in custom:
        if not isinstance(a, dict) or not isinstance(a.get("key"), str) or not re.fullmatch(r"custom-[a-z0-9-]{1,80}", a["key"]) or a["key"] in custom_ids or not text(a.get("name"), 60) or a.get("type") not in ("storage", "table", "printer", "purifier"):
            raise ValueError("추가한 자산이 올바르지 않습니다.")
        for k, low, high in (("x", 14, 277), ("y", 18, 731)):
            if type(a.get(k)) not in (int, float) or not math.isfinite(a[k]) or not low <= a[k] <= high:
                raise ValueError("자산 위치가 지도 밖입니다.")
        if a.get("w") != 20 or a.get("d") != 24:
