...
office_map = components.declare_component(
    "office_atlas_shared",
    path=str(ROOT / "frontend"),
)
...
def main():
    ...
    try:
        ...
        st.error(
            "Secrets 설정을 읽지 못했습니다. "
            "앱 Settings → Secrets의 저장 상태와 형식을 확인해 주세요."
        )
        st.stop()

    if missing:
        st.error("Secrets에 필요한 설정이 없습니다: " + ", ".join(missing))
        st.info(
            "OFFICE_SHEET_ID와 OFFICE_PASSWORD는 "
            "[gcp_service_account]보다 위에 작성해 주세요."
        )
        st.stop()
    ...
    def live_map():
        ...
        with st.expander("최근 수정 기록"):
            try:
                history = list(reversed(shared.refresh().history[-30:]))
                st.dataframe(
                    history,
                    hide_index=True,
                    width="stretch",
